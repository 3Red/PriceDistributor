...
class RedisWriter(object):
    ...
    def publish_book(self, symbol, book):
        self.redis.setex('stocks:{}'.format(symbol), self.expire, json.dumps(book))

    def update_expire(self):
        with self.redis.pipeline() as pipe:
            for symbol in self.books.iterkeys():
                pipe.expire('stocks:{}'.format(symbol), self.expire)
            pipe.execute()
    ...
    def run(self):
        while True:
            try:
                data = self.listener.recv(1024 * 2)
                self.convert(data)
